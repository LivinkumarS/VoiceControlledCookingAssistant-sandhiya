...
app.post("/api/recipe", async (req, res) => {
  try {
    ...
    let recipe = await fetchFromMealDB(keyword) || 
...
async function fetchFromMealDB(keyword) {
  try {
    ...
    if (ingredientResponse.data.meals && ingredientResponse.data.meals.length > 0) {
...
function cleanKeyword(keyword) {
  ...
  const recipeTypes = ['omelette', 'curry', 'salad', 'soup', 'stir fry', 'stir-fry', 'pasta', 'rice', 'sandwich', 'burger'];
...
function generateSmartIngredients(keyword) {
  const commonBases = {
    'omelette': ['3 large eggs', '1 tbsp milk or water', 'Salt and pepper to taste', '1 tbsp butter or oil'],
    'curry': ['500g main protein/vegetables', '1 onion, finely chopped', '2 garlic cloves, minced', '1 tbsp curry powder', '200ml coconut milk', '1 tbsp oil'],
    'salad': ['Mixed greens or base vegetable', '1 tbsp olive oil', '1 tsp lemon juice or vinegar', 'Salt and pepper to taste', 'Optional: herbs, nuts, cheese'],
    'soup': ['500g main ingredient', '1 liter vegetable/chicken stock', '1 onion, chopped', '2 garlic cloves', '1 tbsp oil', 'Herbs and spices to taste'],
    'pasta': ['200g pasta', '2L water for boiling', '1 tbsp salt', 'Sauce ingredients based on recipe', 'Grated cheese for serving'],
    'rice': ['1 cup rice', '2 cups water', '1 tbsp oil or butter', 'Salt to taste', 'Additional ingredients as per recipe']
  };
...
function getDefaultImage(keyword) {
  const images = {
    'omelette': 'https://www.themealdb.com/images/media/meals/xxyupu1468262513.jpg',
...
function formatRecipe(recipe) {
  ...
  if (recipe.cookingTime) {
    formatted += `⏱️ Cooking Time: ${recipe.cookingTime} | 📊 Difficulty: ${recipe.difficulty || 'Medium'}\n\n`;
  }
